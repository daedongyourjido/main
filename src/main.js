...
function AfterLogin(props){
  ...
  return (
    <div className="bar" style={{display:'flex', flexDirection:'row', justifyContent:'right', marginTop:'20px'}}>
        <SearchField />
        <img src={propic} style={{width:'40px', height:'40px', borderRadius:'100%', marginRight:'16px', cursor: 'pointer'}} onClick={()=>{navigate('/setting')}} />
        <p style={{fontSize:'18px', cursor: 'pointer'}} onClick={()=>{navigate('/setting')}} >{localStorage.getItem('name')}</p>
